...
const NewServiceOrderScreen = ({ route, navigation }) => {
    // const { isp, usuarioId } = route.params;
    const { usuarioId } = route.params;
    const { isDarkMode } = useTheme();
    const styles = getStyles(isDarkMode);

    // Aquí guardaremos el id_isp que trae la ruta /datos-usuario
    const [userIspId, setUserIspId] = useState(null);
    ...
    console.log('Ususario:' + usuarioId);
    ...
    /////////////////////////////////////////////////////////////////////////////
    // 2. Una vez tengamos userIspId, ejecutamos las demás peticiones 
    //    que dependen de userIspId
    /////////////////////////////////////////////////////////////////////////////
    useEffect(() => {
        if (!userIspId) return; // Si no tenemos userIspId aún, no cargamos nada más

        // Cargar tipos de órdenes
        const fetchOrderTypes = async () => {
            try {
                const response = await fetch(`https://wellnet-rd.com:444/api/order-types?id_isp=${userIspId}`);
                const data = await response.json();
                if (!response.ok) {
                    console.error('Error en order-types:', data.error);
        ...
        const fetchClientes = async () => {
            try {
                ...
                const data = await response.json();
                if (!response.ok) {
                    console.error('Error en lista-clientes:', data.error);
                    Alert.alert('Error', data.error);
                } else {
        ...
        // Llamamos a estas dos funciones ahora que sí tenemos el userIspId
        fetchOrderTypes();
        fetchClientes();
    }, [userIspId, usuarioId]);
    ...
    const fetchServicios = async () => {
        try {
            const response = await fetch(`https://wellnet-rd.com:444/api/lista-servicios`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ isp_id: userIspId, usuarioId }),
            });
            const data = await response.json();
            if (!response.ok) {
                console.error('Error en el servidor:', data.error);
    ...
    const fetchTecnicos = async () => {
        try {
            const response = await fetch('https://wellnet-rd.com:444/api/usuarios/usuarios', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ ispId: userIspId }),
            });
    ...
    <View style={styles.orderTypeButton}>
        <Text
            ...
            onPress={() => {
                fetchConexiones(form.idCliente, userIspId, setConexiones, setFilteredConexiones); // Solicitar conexiones al abrir el modal
