...
const DetalleCortesScreen = ({ route, navigation }) => {
    ...
    useEffect(() => {
        const fetchCortesPorDia = async () => {
            try {
                ...
                const response = await fetch('https://wellnet-rd.com/api/usuarios/cortes-usuario-por-dia-especifico', { 
                    method: 'POST',
                    headers: {
                ...
                if (response.ok) {
                    setCortes(data.reverse());  // Invertir el orden de la lista
                } else {
                    throw new Error(data.message || 'Error al obtener los cortes del día');
                }
            } catch (error) {
                console.error('Error al obtener los cortes del día:', error);
                Alert.alert('Error', error.message || 'Error al obtener los cortes del día.');
            } finally {
                setLoading(false);
            }
        };
    ...
    return (
        <View style={styles.container}>
            <Text style={styles.title}>Cortes de servicio del día:</Text>
            ...
            {loading ? (
                ...
                <FlatList
                    ...
                    ListEmptyComponent={<Text style={styles.emptyText}>No hay cortes registrados para este día.</Text>}
                />
            )}
        </View>
    );
...
export default DetalleCortesScreen;
