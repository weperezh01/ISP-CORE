...
const DetalleLlamadasScreen = ({ route, navigation }) => {
    ...
    useEffect(() => {
        const fetchLlamadasPorDia = async () => {
            try {
                const formattedDate = new Date(dia).toISOString().split('T')[0];
        
                const response = await fetch('https://wellnet-rd.com/api/usuarios/llamadas-usuario-por-dia-especfico', { 
                    method: 'POST',
                    headers: {
                ...
                if (response.ok) {
                    const llamadasInvertidas = data.reverse();
                    setLlamadas(llamadasInvertidas);
                } else {
                    throw new Error(data.message || 'Error al obtener las llamadas del día');
                }
            } catch (error) {
                console.error('Error al obtener las llamadas del día:', error);
                Alert.alert('Error', error.message || 'Error al obtener las llamadas del día.');
            } finally {
                setLoading(false);
            }
        };
    ...
    const renderItem = ({ item }) => (
        ...
        >
            ...
            {/* <Text style={styles.itemDetail}>Correo: {item.cliente?.correo_elect}</Text> */}
    ...
    return (
        <View style={styles.container}>
            <Text style={styles.title}>Llamadas del día:</Text>
            ...
            {loading ? (
                ...
                <FlatList
                    data={llamadas}
                    keyExtractor={(item) => item.usuario_llamada_id.toString()}
                    renderItem={renderItem}
                    ListEmptyComponent={<Text style={styles.emptyText}>No hay llamadas registradas para este día.</Text>}
                />
            )}
        </View>
    );
...
export default DetalleLlamadasScreen;
